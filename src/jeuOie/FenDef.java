...
public class FenDef extends JFrame implements ActionListener
{	
	private JTextField rpse[]; 
	private String pass; 
	private Definition Case;
	
	public FenDef (String t1, String t2,Definition cas)
	{
		/*----------------	GRAPHIQUE -------------*/
	...
	{		
		...
		if(aRpdu){
			...
			{  
				j.incrementer( def.getPOINTGAGNE());
				info.setScores();
				Case.add();
				info.setSomme(4);
			...
			{
				j.incrementer( def.getPOINTPERDU() );
				info.setScores();
				JOptionPane.showMessageDialog(new Frame(),
